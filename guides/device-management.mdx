---
title: "Discover and Manage Roku and Fire TV Devices"
...
The scan runs for 4 seconds (`SSDP_TIMEOUT`) while it waits for devices to respond. For each Roku IP that replies, PC Caster calls `GET http://<ip>:8060/query/device-info` to retrieve the device's user-assigned name (or model name as a fallback). Fire TV devices are listed as `Fire TV (<ip>)`.
