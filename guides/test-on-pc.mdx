---
title: "Test the Proxied Stream on Your PC Before Casting"
...
2. **Proxied URL is generated** — The proxy constructs a local URL that routes all requests through itself, injecting the captured `Referer` and a real Chrome TLS fingerprint on every outbound request to the CDN:
   ```
   http://<PC-LAN-IP>:8011/p.m3u8?u=<b64 stream url>&r=<b64 referer>
   ```

3. **VLC is launched** — PC Caster checks for VLC at the following paths:
   ```
...
4. **No VLC found** — If VLC isn't installed in the default location, the proxied URL is copied to your clipboard and a dialog explains how to use it manually.
