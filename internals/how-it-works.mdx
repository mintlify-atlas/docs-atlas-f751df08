...
description: "A technical overview of the PC Caster pipeline: stream detection via Playwright, the local HLS proxy with Chrome TLS spoofing, and the Roku ECP channel control flow."
---
...
`stream_finder.py` opens a **visible** Chromium browser via Playwright using `find_streams_interactive`. As soon as the browser navigates to the streaming page, it attaches a request listener with `page.on('request', _record)` to the main page and to every new tab the site opens.
...
The user clicks the server link they want inside the browser. The moment the video player fires its first `.m3u8` request, PC Caster captures it and shows it in the scanner modal. Ad and popup tabs are automatically closed in the background loop so they don't bury the real player window.
...
PC Caster finds Roku devices on the LAN automatically, without requiring the user to know the IP address. It sends a UDP M-SEARCH to the multicast address `239.255.255.250:1900` with the Roku-specific search target:

```python pc_caster.py
SSDP_ADDR = "239.255.255.250"
SSDP_PORT = 1900
# Search target sent in the M-SEARCH:  ST: roku:ecp
```
