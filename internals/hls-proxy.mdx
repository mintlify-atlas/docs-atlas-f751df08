...
description: "The hls_proxy.py module implements a local ThreadingHTTPServer that re-fetches HLS streams with a Chrome TLS fingerprint and rewrites .m3u8 playlists to stay proxied."
---
...
- **Bare lines (segment paths):** treated as relative or absolute segment URIs. Each is resolved to an absolute URL with `urljoin(base_url, line)` and then wrapped in a proxy URL.

### Example

```m3u8 Before rewriting
#EXTM3U
